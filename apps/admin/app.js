...
function sidebar() {
  ...
  return `<aside class="sidebar"><a href="/admin" class="admin-brand"><span class="admin-mark">XX</span><span>XiuXian<small>Operations console · v${esc(state.version ?? '1.0.1')}</small></span></a><span class="side-label">Workspace</span><nav class="side-nav">${links.map(([view, label]) => `<button class="${state.view === view ? 'active' : ''}" data-action="navigate" data-view="${view}">${icons[view]}${label}</button>`).join('')}</nav><div class="sidebar-foot"><strong>${esc(state.user?.firstName ?? '管理员')}</strong>Telegram ID ${esc(state.user?.telegramId ?? '')}<br/>数字商品交付系统 · v${esc(state.version ?? '1.0.1')}</div></aside>`;
}
