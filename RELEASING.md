...
XiuXian 使用严格递增的 `1.0.N` 版本号。远程仓库只保留 `main` 分支；所有代码、样式、文档、配置和运维改动都直接提交到 `main`，不建立长期功能分支。买家端构建使用 React + TypeScript + TailwindCSS，官方 Telegram React 绑定包通过项目兼容别名 `@telegram-mini-apps/sdk-react` 引用。

## 版本规则

- 每次发布只将最后一位加 `1`，例如 `1.0.33 -> 1.0.34`。
...
4. 运行 `npm run version:check`、`npm run typecheck`、`npm run build`、串行测试 `node --test --test-concurrency=1 "test/*.test.js"`、语法检查和必要的 HTTP/浏览器验收。
